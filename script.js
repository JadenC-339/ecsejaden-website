  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.querySelector('.skill-progress').style.width = entry.target.dataset.skill === 'Design' ? '85%' : 
                                                                  entry.target.dataset.skill === 'Development' ? '90%' : 
                                                                  entry.target.dataset.skill === 'Photography' ? '75%' : '70%';
      }
    });
  }, { threshold: 0.5 });
